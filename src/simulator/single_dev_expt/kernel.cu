...
namespace gf::simulator::single_dev_expt
{
    ...
    template<>
    __global__ __launch_bounds__(1024) void
    HaloBlockingDynamicL2D3Q27InplaceMiddleKernel<true>(const HaloBlockingDynamicL2InplaceParam __grid_constant__ param)
    {
        const idx_t blkx = blockDim.x * blockIdx.x + threadIdx.x;
        const idx_t blky = blockDim.y * blockIdx.y + threadIdx.y;
        const idx_t blkz = blockDim.z * blockIdx.z + threadIdx.z;
    ...
    template<>
    __global__ __launch_bounds__(1024) void 
    HaloBlockingDynamicL2D3Q27InplaceLastKernel<false>(const HaloBlockingDynamicL2InplaceParam __grid_constant__ param)
    {
        const idx_t blkx = blockDim.x * blockIdx.x + threadIdx.x;
        const idx_t blky = blockDim.y * blockIdx.y + threadIdx.y;
        const idx_t blkz = blockDim.z * blockIdx.z + threadIdx.z;
    ...
    template<>
    __global__ __launch_bounds__(1024) void 
    HaloBlockingDynamicL2D3Q27InplaceLastKernel<true>(const HaloBlockingDynamicL2InplaceParam __grid_constant__ param)
    {
        const idx_t blkx = blockDim.x * blockIdx.x + threadIdx.x;
        const idx_t blky = blockDim.y * blockIdx.y + threadIdx.y;
        const idx_t blkz = blockDim.z * blockIdx.z + threadIdx.z;
    ...
    __global__ __launch_bounds__(1024) void
    HaloBlockingDynamicL2D3Q27InplaceDumpKernel(const HaloBlockingDynamicL2InplaceParam __grid_constant__ param)
    {
        const idx_t blkx = blockDim.x * blockIdx.x + threadIdx.x;
        const idx_t blky = blockDim.y * blockIdx.y + threadIdx.y;
        const idx_t blkz = blockDim.z * blockIdx.z + threadIdx.z;
    ...
    __global__ __launch_bounds__(1024) void
    HaloBlockingStaticL2D3Q27InplaceKernel(const HaloBlockingStaticL2InplaceParam __grid_constant__ param)
    {
        const idx_t blkx = blockDim.x * blockIdx.x + threadIdx.x;
        const idx_t blky = blockDim.y * blockIdx.y + threadIdx.y;
        const idx_t blkz = blockDim.z * blockIdx.z + threadIdx.z;
    ...
    __global__ __launch_bounds__(1024) void
    HaloBlockingStaticL2D3Q27InplaceDumpKernel(const HaloBlockingStaticL2InplaceParam __grid_constant__ param)
    {
        const idx_t blkx = blockDim.x * blockIdx.x + threadIdx.x;
        const idx_t blky = blockDim.y * blockIdx.y + threadIdx.y;
        const idx_t blkz = blockDim.z * blockIdx.z + threadIdx.z;
    ...
    __global__ __launch_bounds__(1024) void
    StaticD3Q27PullKernel(const StaticPullParam __grid_constant__ param)
    {
        const idx_t x = blockDim.x * blockIdx.x + threadIdx.x;
        const idx_t y = blockDim.y * blockIdx.y + threadIdx.y;
        const idx_t z = blockDim.z * blockIdx.z + threadIdx.z;
        const idx_t nx = gridDim.x * blockDim.x;
        const idx_t ny = gridDim.y * blockDim.y;
        const idx_t nz = gridDim.z * blockDim.z;

        const idx_t i = (ny * z + y) * nx + x;
        const idx_t n = nx * ny * nz;
        const flag_t flagi = param.glbFlagBuf[i];
        real_t fni[27];
        
        if((flagi & LOAD_DDF_BIT)!=0)
        {
            const idx_t ndx = (x==0)    ? 0 : -1;
            const idx_t pdx = (x==nx-1) ? 0 :  1;
            const idx_t ndy = (y==0)    ? 0 : -nx;
            const idx_t pdy = (y==ny-1) ? 0 :  nx;
            const idx_t ndz = (z==0)    ? 0 : -nx * ny;
        ...
        if((flagi & EQU_DDF_BIT)!=0)
        {
            const real_t rhoi = param.glbRhoBuf[i];
    ...
    __global__ __launch_bounds__(1024) void
    StaticD3Q27PullDumpKernel(const StaticPullParam __grid_constant__ param)
    {
        const idx_t x = blockDim.x * blockIdx.x + threadIdx.x;
        const idx_t y = blockDim.y * blockIdx.y + threadIdx.y;
        const idx_t z = blockDim.z * blockIdx.z + threadIdx.z;
        ...
        {
            param.glbRhoBuf[i] = rhoi;
        }

        if((flagi & DUMP_VX_BIT)!=0)
        {
            param.glbVxBuf[i] = vxi;
        }

        if((flagi & DUMP_VY_BIT)!=0)
        {
            param.glbVyBuf[i] = vyi;
        }

        if((flagi & DUMP_VZ_BIT)!=0)
        {
            param.glbVzBuf[i] = vzi;
        }
    }
}